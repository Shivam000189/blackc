...
export const RegionDonutChart: React.FC<RegionDonutChartProps> = ({
  data,
  loading = false,
  error = null,
}) => {
  // Take top 3 or default values
  const defaultItems = [
    { label: "North America", value: 60, color: "#4355b9" },
    { label: "Asia & Pacific", value: 20, color: "#f0c040" },
    { label: "Europe & Africa", value: 20, color: "#e05070" },
  ];
  ...
  if (data && data.length >= 3) {
    ...
    items = [
      {
        label: data[0].region || "North America",
        value: Math.round((data[0].count / total) * 100),
        color: "#4355b9",
      },
      {
        label: data[1].region || "Asia",
        value: Math.round((data[1].count / total) * 100),
        color: "#f0c040",
      },
      {
        label: data[2].region || "Europe",
  ...
  // SVG Donut calculation (circumference = 2 * PI * 35 = ~220)
  const c = 220;
